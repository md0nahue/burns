...
func main() {
	...
	if event.ProjectID == "" {
		fmt.Fprintf(os.Stderr, "❌ project_id is required\n")
		os.Exit(1)
	}
	...
	if event.SegmentID != "" && len(event.Images) > 0 {
		...
		fmt.Fprintf(os.Stderr, "❌ Invalid event format\n")
		os.Exit(1)
	}
}
...
func uploadToS3(localPath, s3Key string) error {
	...
	cmd := exec.Command("aws", "s3", "cp", localPath, fmt.Sprintf("s3://%s/%s", bucket, s3Key))
	cmd.Stderr = os.Stderr
	return cmd.Run()
}
...
func downloadFromS3(s3Key, localPath string) error {
	...
	cmd := exec.Command("aws", "s3", "cp", fmt.Sprintf("s3://%s/%s", bucket, s3Key), localPath)
	cmd.Stderr = os.Stderr
	return cmd.Run()
}
